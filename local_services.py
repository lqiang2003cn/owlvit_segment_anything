...
def owl_vit():
    ...
    if not request.json or 'image' not in request.json:
        abort(400)
    im_array_from_request = np.array(request.json['image'])
    image = Image.fromarray(im_array_from_request, mode='RGB')
    ...
    # results = owl_vit_processor.post_process_object_detection(outputs=outputs, threshold=0.0, target_sizes=target_sizes.to(device))
